import { useStripe, GooglePayButton, useGooglePay } from '@stripe/stripe-react-native';
import React, { useEffect, useState } from 'react';
import { TextInput, Alert,View, Text, TouchableOpacity, Button, Image } from 'react-native';
import * as i18n from '../../i18n/i18n';
import axios from '../../component/AxiosNet';
import PaymentScreen from '../../component/PaymentScreen';
import moment from 'moment-timezone';

import { SafeAreaView } from 'react-native-safe-area-context';
import { useSelector } from 'react-redux';

import {uri} from '@env';


//
const ShopCheckOutScreen = ({ route,navigation }) => {
  i18n.setI18nConfig();
  const { userId } = useSelector(state => state.loginReducer);
  const [initialized, setInitialized] = useState(false);
  const [loading, setLoading] = useState(false);

  const [customer, setCustomer] = useState('');
  const [email, setEmail] = useState('');
  const [address, setAddress] = useState('');


  async function doPay() {
    /*if(!customer){
      alert('Please input your name!');
      return;
    }
    if(!email){
      alert('Please input your email!');
      return;
    }
    if(!address){
      alert('Please input your address!');
      return;
    }*/
    navigation.navigate('Paypal',{customer:customer,email:email,address:address,coupon:route.params.coupon, date:route.params.date})
  }

  if (Platform.OS == 'android') {
    const {
      initGooglePay,
      presentGooglePay,
      loading,
      createGooglePayPaymentMethod,
    } = useGooglePay();
    const fetchPaymentIntentClientSecret = async () => {
      const { data } = await axios.post(`cart/checkout`,
        {
          uid: userId,
          date: moment(route.params.date).zone('+0800').format()
        });
      const clientSecret = data['paymentIntent'];
      return clientSecret;
    };

    const pay = async () => {
      // 2. Fetch payment intent client secret
      const clientSecret = await fetchPaymentIntentClientSecret();

      // 3. Open Google Pay sheet and proceed a payment
      const { error } = await presentGooglePay({
        clientSecret,
        forSetupIntent: false,
      });

      if (error) {
        Alert.alert(error.code, error.message);
        return;
      }
      Alert.alert('Success', 'The payment was confirmed successfully.');
      navigation.navigate('UserScreen',{screen:'Order',initial: false});
      
      setInitialized(false);
    }


    /*
      As an alternative you can only create a paymentMethod instead of confirming the payment.
    */
    const createPaymentMethod = async () => {
      const { error, paymentMethod } = await createGooglePayPaymentMethod({
        amount: 12,
        currencyCode: 'USD',
      });

      if (error) {
        Alert.alert(error.code, error.message);
        return;
      } else if (paymentMethod) {

        Alert.alert(
          'Success',
          `The payment method was created successfully. paymentMethodId: ${paymentMethod.id}`
        );
      }
      
      setInitialized(false);
    };

    // 1. Initialize Google Pay
    const initialize = async () => {
      const { error } = await initGooglePay({
        testEnv: true,
        merchantName:"NIHONKAI日本海",
        countryCode: 'HK',
        billingAddressConfig: {
          format: 'FULL',
          isPhoneNumberRequired: true,
          isRequired: false,
        },
        existingPaymentMethodRequired: false,
        isEmailRequired: true,
      });

      if (error) {
        Alert.alert(error.code, error.message);
        return;
      }
      setInitialized(true);
    };

    return (
    <PaymentScreen onInit={initialize}>
      <SafeAreaView style={{justifyContent:'center',alignItems:'center'}}>
        {/*<GooglePayButton
          disabled={!initialized || loading}
          style={{
            width: 152,
            height: 40,
            marginTop:'10%'
          }}
          type="pay"
          onPress={pay}
        />*/}

       {/* <View style={{width: '90%', flexDirection: 'column', marginLeft: '5%', marginBottom: '5%'}}>
          <View style={{width: '25%', justifyContent: 'center', alignContent: 'center'}}>
            <Text style={{fontSize: 18}}>Name</Text>
          </View>
          <TextInput style={{fontSize: 15, borderColor: '#ccc', backgroundColor: '#fff', borderWidth: 1, padding: 15, borderRadius: 10, marginTop: 5}}
              onChangeText={test => {
                setCustomer(test);
              }} value={customer}
              maxLength={30} placeholder="please input name!"
          />
        </View>

        <View style={{width: '90%', flexDirection: 'column', marginLeft: '5%', marginBottom: '5%'}}>
          <View style={{width: '25%', justifyContent: 'center', alignContent: 'center'}}>
            <Text style={{fontSize: 18}}>Email</Text>
          </View>
          <TextInput style={{fontSize: 15, borderColor: '#ccc', backgroundColor: '#fff', borderWidth: 1, padding: 15, borderRadius: 10, marginTop: 5}}
              onChangeText={test => {
                setEmail(test);
              }} value={email}
              maxLength={30} placeholder="please input email!"
          />
        </View>

        <View style={{width: '90%', flexDirection: 'column', marginLeft: '5%', marginBottom: '10%'}}>
          <View style={{width: '25%', justifyContent: 'center', alignContent: 'center'}}>
            <Text style={{fontSize: 18}}>Address</Text>
          </View>
          <TextInput style={{fontSize: 15, borderColor: '#ccc', backgroundColor: '#fff', borderWidth: 1, padding: 15, borderRadius: 10, marginTop: 5}}
              onChangeText={test => {
                setAddress(test);
              }} value={address}
              maxLength={30} placeholder="please input address!"
          />
            </View>*/}

        <TouchableOpacity style={{height:80,width:'100%',borderRadius:100, marginTop:'20%'}} onPress={()=>doPay()}>
          <Image style={{height:'100%',width:'100%',borderRadius:5}} resizeMode='contain' source={{uri:'data:image/png;base64,'}}></Image>
        </TouchableOpacity>

        {/*<TouchableOpacity style={{height:80,width:'100%',borderRadius:100}} onPress={()=>navigation.navigate('Storepay',{coupon:route.params.coupon, date:route.params.date})}>
          <View style={{display:'flex',justifyContent:'center',alignItems:'center'}}><Text style={{borderColor:'#6a6a6a', borderWidth:2, width:110, height:40,lineHeight:40,borderRadius:10,textAlign:'center'}}>{i18n.t('pay in store')}</Text></View>
        </TouchableOpacity>*}

        {/*<TouchableOpacity style={{height:80,width:'100%',borderRadius:100, marginTop:'20%'}} onPress={()=>navigation.navigate('Stripe',{coupon:route.params.coupon, date:route.params.date})}>
          <Image style={{height:'100%',width:'100%',borderRadius:100}} resizeMode='contain' source={{uri:uri + 'Public/image/stripe.png?rnd=' + Math.random() * 10000}}></Image>
        </TouchableOpacity>*/}
      </SafeAreaView>
      </PaymentScreen>
    );

  } else {
    const { initPaymentSheet, presentPaymentSheet } = useStripe();

    const fetchPaymentSheetParams = async () => {
      const { data } = await axios.post(`cart/checkout`,
        {
          uid: userId,
          coupon:route.params.coupon,
          date: moment(route.params.date).zone('+0800').format()
        });
      const paymentIntent = data['paymentIntent'];
      const ephemeralKey = data['ephemeralKey'];
      const customer = data['customer'];
      return {
        paymentIntent,
        ephemeralKey,
        customer,
      };
    };

    const initializePaymentSheet = async () => {
      const {
        paymentIntent,
        ephemeralKey,
        customer,
      } = await fetchPaymentSheetParams();

      const { error } = await initPaymentSheet({
        customerId: customer,
        customerEphemeralKeySecret: ephemeralKey,
        paymentIntentClientSecret: paymentIntent,
        allowsDelayedPaymentMethods: true,
        applePay: true,
        googlePay: true,
        currencyCode: 'HKD',
        merchantCountryCode: 'HK',
        merchantDisplayName: 'maltcigar',
        testEnv: false,
      });

      if (!error) {
        setLoading(true);
      }
    };

    const openPaymentSheet = async () => {
      const { error } = await presentPaymentSheet();
      if (error) {
        console.log(error)
      } else {
        Alert.alert('Success', 'Your order is confirmed!',{ text: "OK", onPress: () => console.log("OK Pressed") });
        navigation.navigate('UserScreen',{screen:'Order',initial: false});
        
      }
    };

    useEffect(() => {
      try {
        initializePaymentSheet();
      } catch (e) {
        console.log(e);
      }

      return function cleanup() {
        initializePaymentSheet();
      }
    }, []);

    return (
      <SafeAreaView>
        {/* <Button
          variant="primary"
          disabled={!loading}
          title="Credit card Or Apple Pay"
          onPress={openPaymentSheet}
        /> */}

        {/*
        <View style={{width: '90%', flexDirection: 'column', marginLeft: '5%', marginBottom: '5%'}}>
          <View style={{width: '25%', justifyContent: 'center', alignContent: 'center'}}>
            <Text style={{fontSize: 18}}>Name</Text>
          </View>
          <TextInput style={{fontSize: 15, borderColor: '#ccc', backgroundColor: '#fff', borderWidth: 1, padding: 15, borderRadius: 10, marginTop: 5}}
              onChangeText={test => {
                setCustomer(test);
              }} value={customer}
              maxLength={30} placeholder="please input name!"
          />
        </View>

        <View style={{width: '90%', flexDirection: 'column', marginLeft: '5%', marginBottom: '5%'}}>
          <View style={{width: '25%', justifyContent: 'center', alignContent: 'center'}}>
            <Text style={{fontSize: 18}}>Email</Text>
          </View>
          <TextInput style={{fontSize: 15, borderColor: '#ccc', backgroundColor: '#fff', borderWidth: 1, padding: 15, borderRadius: 10, marginTop: 5}}
              onChangeText={test => {
                setEmail(test);
              }} value={email}
              maxLength={30} placeholder="please input email!"
          />
        </View>

        <View style={{width: '90%', flexDirection: 'column', marginLeft: '5%', marginBottom: '10%'}}>
          <View style={{width: '25%', justifyContent: 'center', alignContent: 'center'}}>
            <Text style={{fontSize: 18}}>Address</Text>
          </View>
          <TextInput style={{fontSize: 15, borderColor: '#ccc', backgroundColor: '#fff', borderWidth: 1, padding: 15, borderRadius: 10, marginTop: 5}}
              onChangeText={test => {
                setAddress(test);
              }} value={address}
              maxLength={30} placeholder="please input address!"
          />
            </View>*/}

        <TouchableOpacity style={{height:80,width:'100%',borderRadius:100}} onPress={()=>doPay()}>
          <Image style={{height:'100%',width:'100%',borderRadius:100}} resizeMode='contain' source={{uri:'data:image/png;base64,'}}></Image>
        </TouchableOpacity>

        {/*<TouchableOpacity style={{height:80,width:'100%',borderRadius:100}} onPress={()=>navigation.navigate('Storepay',{coupon:route.params.coupon, date:route.params.date})}>
          <View style={{display:'flex',justifyContent:'center',alignItems:'center'}}><Text style={{borderColor:'#6a6a6a', borderWidth:2, width:110, height:40,lineHeight:40,borderRadius:10,textAlign:'center'}}>{i18n.t('pay in store')}</Text></View>
        </TouchableOpacity>*/}

        {/*<TouchableOpacity style={{height:80,width:'100%',borderRadius:100}} onPress={()=>navigation.navigate('Stripe',{coupon:route.params.coupon, date:route.paråams.date})}>
          <Image style={{height:'100%',width:'100%',borderRadius:100}} resizeMode='contain' source={{uri:uri + 'Public/image/stripe.png?rnd=' + Math.random() * 10000}}></Image>
        </TouchableOpacity>*/}

      </SafeAreaView>
    );
  }
}
export default ShopCheckOutScreen;
